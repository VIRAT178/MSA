...
async function ensureSeedAccounts() {
  const seedAccounts = [
    {
      role: "admin",
      email: (process.env.ADMIN_EMAIL || "director@malwasportsacademy.org").toLowerCase(),
      password: process.env.ADMIN_PASSWORD || "MSAadmin123!",
      name: process.env.ADMIN_NAME || "MSA Director",
      phone: process.env.ADMIN_PHONE || "+91 95000-MSA-ID",
      sportsProfile: {
        sport: "cricket",
        streak: 0,
        maxVelocity: "28 km/h",
        ...
        proteinTarget: "120g",
        carbsTarget: "280g",
        coachingNote: "Admin seed account for Malwa Sports Academy.",
        attendanceSessions: 0,
        attendanceAttended: 0,
      },
      createdAt: new Date(),
      passwordLabel: "MSAadmin123!"
    },
    {
      role: "coach",
      email: (process.env.COACH_EMAIL || "coach.satnam@malwasportsacademy.org").toLowerCase(),
      password: process.env.COACH_PASSWORD || "MSAcoach123!",
      name: process.env.COACH_NAME || "Coach Satnam Gill",
      phone: process.env.COACH_PHONE || "+91 95000-COACH-ID",
      sportsProfile: {
        sport: "cricket",
        streak: 0,
        maxVelocity: "32 km/h",
        ...
        proteinTarget: "125g",
        carbsTarget: "290g",
        coachingNote: "Coach seed account for Malwa Sports Academy.",
        attendanceSessions: 0,
        attendanceAttended: 0,
      },
      createdAt: new Date(),
      passwordLabel: "MSAcoach123!"
    }
  ];
  ...
  try {
    for (const account of seedAccounts) {
      const existing = dbConnected
        ...
        : inMemoryDb.users.find((u) => u.email === account.email);
      ...
      const record = {
        ...
        role: account.role,
        phone: account.phone,
        sportsProfile: account.sportsProfile,
      ...
      if (dbConnected) {
        ...
        record._id = `seed_${account.role}_${Date.now()}`;
        inMemoryDb.users.push(record);
        console.log(`✅ Seeded ${account.role} account in memory: ${account.email}`);
      }
    }
  } catch (error: any) {
    console.error("Failed to ensure seed accounts:", error.message);
  }
}
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const allowedRegistrationRoles = ["student", "parent"];
    ...
    if (!allowedRegistrationRoles.includes(role)) {
      return res.status(400).json({ error: "Registration is only available for student and parent accounts." });
    }
...
app.post("/api/contact", async (req, res) => {
  try {
    const { name, phone, query, email } = req.body;

    if (!name || !phone || !query) {
      return res.status(400).json({ error: "Missing name, phone, or query text" });
    }

    const contactRecord = { name, phone, query, email: email || "", createdAt: new Date() };
    const dbConnected = mongoose.connection.readyState === 1;
