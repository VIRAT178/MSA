...
export default function Dashboard({ initialRole, currentUser, onProfileUpdate }: DashboardProps) {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      const [admRes, evtRes, conRes] = await Promise.all([
        fetch(apiPath("/api/admissions")),
        fetch(apiPath("/api/event-registrations")),
        fetch(apiPath("/api/contacts"))
      ]);
  ...
  const handleAdmissionAction = async (ticketId: string, status: "Accepted" | "Denied") => {
    try {
      const res = await fetch(apiPath("/api/admissions/action"), {
        method: "POST",
  ...
  const handleEventAction = async (token: string, status: "Accepted" | "Denied") => {
    try {
      const res = await fetch(apiPath("/api/event-register/action"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-3 shadow-sm">
                ...
                <div className="text-3xl font-black font-title text-zinc-900">
                  {admissions.length} <span className="text-xs text-zinc-500 font-black ml-1">APPLICATIONS</span>
                </div>
                <p className="text-[11px] text-zinc-400 font-bold uppercase">Live trial applications tracked</p>
              </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-3 shadow-sm">
                ...
                <div className="text-3xl font-black font-title text-zinc-900">
                  {events.length} <span className="text-xs text-zinc-500 font-black ml-1">REGISTRATIONS</span>
                </div>
                <p className="text-[11px] text-zinc-400 font-bold uppercase">Upcoming events being managed</p>
              </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-3 shadow-sm">
                ...
                <div className="text-3xl font-black font-title text-zinc-900">
                  {contacts.length} <span className="text-xs text-red-600 font-bold ml-1">MESSAGES</span>
                </div>
                <p className="text-[11px] text-zinc-400 font-bold uppercase">Visitor questions waiting for review</p>
              </div>

              <div className="rounded-2xl border border-zinc-200 bg-white p-6 space-y-3 shadow-sm">
                <div className="flex items-center justify-between text-zinc-500 text-xs font-black uppercase">
                  <span>Approved Trials</span>
                  <TrendingUp className="h-5 w-5 text-green-600" />
                </div>
                <div className="text-3xl font-black font-title text-zinc-900">
                  {admissions.filter(a => a.status === "Accepted").length} <span className="text-xs text-green-600 font-bold ml-1">CONFIRMED</span>
                </div>
                <p className="text-[11px] text-zinc-400 font-bold uppercase">Confirmed trial booking</p>
              </div>
