...
import { MessageSquare, Send, Sparkles, Trophy, RefreshCw, Dumbbell, UserCheck, Flame } from "lucide-react";
...
export default function AICoach() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      content: "Hello! I am Coach Malwa, your AI Sports Advisor at Malwa Sports Academy. Whether you want to optimize your batting stance, master the high-press in football, prepare a championship diet, or train your mind for clutch performance—I am here to coach you. What are we working on today?"
    }
  ]);
  ...
  const handleSend = async (customText?: string) => {
    ...
    try {
      const res = await fetch(apiPath("/api/ai-coach"), {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error("Failed to consult Coach Malwa");
      }
  ...
  const quickPrompts = [
    {
      label: "Cricket Batting Prep",
      text: "Can you design a 15-minute quick warm-up drill to practice facing spin bowlers?"
    },
    {
      label: "Midfielder Stamina",
      text: "What are the best interval sprints to build 90-minute high stamina for football?"
    },
    {
      label: "Shooter Pulse Control",
      text: "How do elite 10m rifle shooters control their heart rate during high-stress matches?"
    },
    {
      label: "Slam-Dunk Vertical Leap",
      text: "Suggest a plyometric gym workout to increase my vertical jump for basketball."
    }
  ];
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-3 mb-10">
          ...
          <h2 className="font-display text-4xl sm:text-5xl font-black uppercase text-zinc-900 tracking-wide">
            CONSULT <span className="text-red-600">COACH MALWA</span>
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-4 bg-zinc-50 border border-zinc-200 p-5 rounded-2xl flex flex-col justify-between space-y-6 shadow-sm">
            ...
            <div className="bg-white p-3.5 rounded-xl border border-zinc-200 flex items-center space-x-2 text-[10px] text-zinc-500 font-mono font-bold shadow-sm">
              ...
              <span>MSA-COACH ENGAGED WITH {selectedSport.toUpperCase()} CONTEXT</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 bg-zinc-50 border border-zinc-200 rounded-2xl flex flex-col h-[520px] overflow-hidden shadow-sm">
            ...
            <div className="bg-white px-4 py-3 border-b border-zinc-200 flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h4 className="text-xs font-black text-zinc-900 uppercase tracking-wider">Coach Malwa AI</h4>
                  <span className="block text-[9px] text-zinc-500 uppercase tracking-widest font-bold">BCCI & FIBA Training Core</span>
                </div>
              </div>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-white border border-zinc-200 rounded-2xl px-4 py-3 text-xs text-zinc-500 flex items-center space-x-2 animate-pulse shadow-sm">
                    ...
                    <span className="font-bold">Coach Malwa is analyzing tactical charts...</span>
                  </div>
                </div>
              )}
              <div ref={chatEndRef} />
            ...
            <div className="p-3 bg-white border-t border-zinc-200 flex items-center gap-2">
              <input
                type="text"
                placeholder="Ask about batting form, dynamic sprints, heart control, swimming mechanics..."
