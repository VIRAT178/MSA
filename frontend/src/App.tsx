...
import { 
  SPORTS_PROGRAMS, COACHES, FEATURED_ATHLETES, UPCOMING_EVENTS, ACHIEVEMENTS, TESTIMONIALS, FACILITIES 
} from "./data";
...
export default function App() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === "login") {
        ...
        setView("dashboard");
        if (pendingEventReg) {
          setEventRegModal(pendingEventReg);
          setPendingEventReg(null);
        }
  ...
  const [pendingEventReg, setPendingEventReg] = useState<string | null>(null);
  ...
  const handleEventRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(apiPath("/api/event-register"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-16 border-b border-zinc-200 pb-6">
                <div>
                  ...
                  <h2 className="font-display text-3xl sm:text-5xl font-black uppercase tracking-wide text-zinc-900 mt-1">
                    12 CORE SPORTS <span className="text-red-600">PROGRAMS</span>
                  </h2>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {SPORTS_PROGRAMS.map((sport) => (
                  <div
                    ...
                    className="group flex flex-col h-[380px] rounded-2xl overflow-hidden border border-zinc-200 bg-white cursor-pointer shadow-sm hover:border-red-600 hover:shadow-lg transition-all duration-300"
                  >
                    {/* Top action image */}
                    <div className="relative h-56 w-full overflow-hidden bg-zinc-100">
                      <img 
                        ...
                        style={{ objectPosition: sport.bgPosition || "center top" }}
                        referrerPolicy="no-referrer"
                      />
                      
                      {/* Floating sports icon badge */}
                      <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm h-10 w-10 rounded-full flex items-center justify-center border border-zinc-100 shadow-md text-red-600 z-10 transition group-hover:scale-110">
                        {getSportIcon(sport.id, "h-5 w-5 text-red-600")}
                      </div>
                    </div>

                    {/* Bottom information section */}
                    <div className="p-5 flex-1 flex flex-col justify-between bg-white text-zinc-900 border-t border-zinc-100">
                      <div className="space-y-1">
                        <span className="text-[9px] bg-red-50 border border-red-100 text-red-600 font-black uppercase px-2.5 py-1 rounded-full tracking-widest inline-block mb-1.5">
                          MSA PATHWAY
                        </span>
                        
                        <h3 className="font-display text-xl font-black uppercase text-zinc-900 tracking-wide flex items-center justify-between">
                    ...
                    <div className="border-t border-zinc-150 pt-3 flex items-center justify-between">
                      ...
                      {evt.registrationOpen ? (
                        <button
                          onClick={() => {
                            if (currentUser) {
                              setEventRegModal(evt.title);
                            } else {
                              setPendingEventReg(evt.title);
                              setLoginModalOpen(true);
                              setAuthMode("login");
                              setAuthError("");
                              setAuthFeedback("Please login or register to complete event registration.");
                            }
                          }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-md animate-fade-in">
          ...
          >
            <div className="text-center space-y-2">
              <Logo className="justify-center" compact={false} showTagline={true} />
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {/* Portal Role switcher (only on login and register) */}
              {authMode !== "forgot" && (
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 bg-zinc-100 p-1 rounded-xl border border-zinc-200">
                    {(authMode === "register"
                      ? (["student", "parent"] as const)
                      : (["admin", "coach", "student", "parent"] as const)
                    ).map((role) => (
                      <button
                        key={role}
                        type="button"
                        onClick={() => {
                          setLoginRole(role);
                          // Auto fill helpers for easy testing
                          if (authMode === "login") {
                            if (role === "admin") setAuthEmail("director@malwasportsacademy.org");
                            else if (role === "coach") setAuthEmail("coach.satnam@malwasportsacademy.org");
                            else if (role === "student") setAuthEmail("arjun.malviya.2026@msa.org");
                            else if (role === "parent") setAuthEmail("sanjay.malviya@domain.com");
                            setAuthPassword("msa123");
                          }
                        }}
                ...
                <div className="text-center">
                  {authMode === "login" ? (
                    <p className="text-[10px] text-zinc-500 font-bold">
                      ...
                      <button 
                        type="button" 
                        onClick={() => { setAuthMode("register"); setAuthError(""); if (loginRole === "admin" || loginRole === "coach") setLoginRole("student"); }}
